    ...
    <div className={`${isClick ? 'w-[400px] activeMobileSidebar' : 'w-[90px]'} relative overflow-  p-2 main-sidebar md:h-[90%] transition-all ease-in-out bg-gray-900 rounded-2xl`}>
        <div className={` absolute start-0  pl-5 h-full `} style={{fontSize: '23px', zIndex: 999}}>
            <div className='flex flex-col w-full h-full justify-center itemce relative'>
                ...
                {
                    isClick ? (
                        <div className='absolute top-0 cursor-pointer mt-5 bg-indigo-600 p-3 rounded-xl bg-opacity-70 w-[45px] flex items-center justify-center' onClick={() => setIsClick(!isClick)}>
                ...
                <div className="flex flex-col items-center gap-5 mt-5 -ml-1">
                    ...
                    <div className="flex group items-center w-full" onClick={() => window.location.href = '/keahlian'}>
                        <i className={`fa fa-code p-3 cursor-pointer w-[50px] icon-sidebars ${path === '/keahlian' ? 'text-indigo-500' : ''}`} />
                        <span className={`${isClick ? 'flex' : 'hidden'} ml-2 text-sm`}>Keahlian</span>
                        <div className={`span-menu hidden ${isClick ? '' : 'group-hover:flex'} transition-all ease-in-out absolute w-20 h-auto justify-center items-center py-1 rounded-xl bg-gray-700 bg-opacity-70 -right-20`} style={{zIndex: 99999, backdropFilter: 'blur(2px)'}}>
                            <span className={` m-0 text-sm`}>Keahlian</span>
                        </div>
                    </div>
                    <div className="flex group items-center w-full" onClick={() => window.location.href = '/layanan'}>
                        <i className={`fa fa-briefcase p-3 cursor-pointer w-[50px] icon-sidebars ${path === '/layanan' ? 'text-indigo-500' : ''}`} />
                    ...
                    <div className="flex group items-center w-full" onClick={() => window.location.href = '/komunitas'}>
                        <i className={`fa fa-comment p-3 cursor-pointer w-[50px] icon-sidebars ${path === '/komunitas' ? 'text-indigo-500' : ''}`} />
